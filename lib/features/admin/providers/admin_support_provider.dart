...
class AdminSupportProvider extends ChangeNotifier {
  ...
  ///
  /// المستمع هنا هو supportRequests، وقراءته مسموحة للمشرف وحده
  /// (isActiveAdmin في القواعد). بقاؤه حيًّا بعد الخروج أو بعد تبديل الحساب
  /// إلى طالب ينتج PERMISSION_DENIED متكررة، والعلاج إيقاف المستمع لا
  /// إضعاف القاعدة — نفس ما فُعل في EnrollmentProvider وAdminTeacherProvider.
  void syncWithAuth({required bool isActiveAdmin}) {
    ...
    final hadAdminState =
        _isAdminSession || _subscription != null || _requests.isNotEmpty;
  ...
  /// يبدأ الاستماع، ويلغي أي اشتراك سابق أولًا.
  ///
  /// الإلغاء قبل البدء — لا `if (_subscription != null) return` — هو نمط
  /// بقية مزوّدات المشرف: فهو يمنع الاشتراك المزدوج ويُبقي إعادة المحاولة
  /// بعد الخطأ عاملة، إذ يبقى الاشتراك غير مُلغى بعد onError فيتحوّل زر
  /// "إعادة المحاولة" إلى لا شيء.
